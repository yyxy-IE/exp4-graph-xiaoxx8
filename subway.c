...
void dijkstra(Graph *g, int start, int *dist, int *prev) {
    ...
    // 主循环
    for (int count = 0; count < g->vertexNum - 1; count++) {
        // 找到未访问节点中距离最小的节点
        int minDist = INT_MAX;
        int u = -1;
        for (int i = 0; i < g->vertexNum; i++) {
...
void shortestPathByTransfer(Graph *g, int start, int end) {
    ...
    // 先保存原始权重
    int *originalWeights = (int*)malloc(sizeof(int) * g->edgeNum);
    int edgeIdx = 0;
    ...
    for (int i = 0; i < g->vertexNum; i++) {
        ...
        while (e) {
            // 保存原始权重
            originalWeights[edgeIdx++] = e->weight;
            // 临时设为1
            e->weight = 1;
            e = e->next;
        }
    ...
    for (int i = 0; i < g->vertexNum; i++) {
        EdgeNode *e = g->vertices[i].firstEdge;
        while (e) {
            e->weight = originalWeights[edgeIdx++];
            e = e->next;
        }
    }
